...
const Dialog = ({
  ...
  const flavor = useSelector((state: any) => {
    return state.flavorSlice.flavor;
  });
  ...
  const handleModalCrud = async (action: any) => {
    ...
    switch (action) {
      case "get":
        ...
        break;
      case "post":
        await server.post("/add", flavor);
        break;
      case "put":
        await server.put("/modify", flavor);
        break;
      case "delete":
        await server.delete("/erase", flavor);
        break;
      default:
